class StockSynchronizationPage {
  elements = {
    ...
    stockFlow: () =>
      cy.get(
        "body > div:nth-child(2) > main:nth-child(3) > div:nth-child(1) > div:nth-child(3) > div:nth-child(1) > div:nth-child(1) > div:nth-child(2) > div:nth-child(2) > div:nth-child(1) > div:nth-child(4)"
      ),
    //stg
    stg_stockFlow: () =>
      cy.get(
    ...
    adminSettings: () =>
      cy.get(
        "body > div:nth-child(2) > main:nth-child(3) > div:nth-child(1) > div:nth-child(2) > div:nth-child(1) > div:nth-child(1) > div:nth-child(2) > div:nth-child(1) > form:nth-child(2) > div:nth-child(1) > div:nth-child(1) > div:nth-child(1) > div:nth-child(1)"
      ),
  ...
  acitvitySettingsTab = () => {
    ...
    cy.intercept("**/activity").as("activityTab");
    ...
    this.elements
      .selectFlow()
      .select("getStockItem")
      .should("have.value", "getStockItem");
    this.elements
      .selectFlow()
      .select("sendStockItem")
      .should("have.value", "sendStockItem");
  ...
  adminSettingsTab = () => {
    //setting admin tab
    cy.intercept("**/admin").as("adminTab");
    this.elements.adminTab().should("be.visible").click();
    cy.wait("@adminTab");
    cy.intercept("**/manual-trigger/*").as("manualTrigger");
    this.elements.triggerManualbtn().should("be.visible").click();
    cy.wait("@manualTrigger");
  };
  stockSettingsTab = () => {
    //customer settings tab
    cy.intercept("**/settings").as("settingsTab");
    ...
    cy.intercept("**/user-integration-flow-settings/*").as("saveSettingsFlow");
    this.elements.saveSettingsbtn().should("be.visible").click();
    cy.wait("@saveSettingsFlow");
  };
  clickOverview = () => {
  ...
  stockSynchronizationFlow = () => {
    ...
    cy.wait("@clickAgree");
    if (environment === "prod") {
      cy.intercept("**/overview").as("stockFlow");
    ...
    this.elements.resumeBtn().should("be.visible").click();
    cy.intercept("**/flag-update").as("resumeFlow");
