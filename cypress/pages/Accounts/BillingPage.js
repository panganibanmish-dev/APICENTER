class BillingPage {
  elements = {
    ...
    lblCurrentPlan: () =>
      cy
        .get("div[id='billing_current-plan'] div[class='card-body']")
        .should("be.visible")
        .contains(
          "You don't have any active subscriptions, set up a new integration."
        ),
    lblFooter: () =>
      cy.get("div[id='billing_current-plan'] div[class='card-footer']"),
    lblPaymentInfo: () =>
      cy.get(
        "div[id='billing_payment-details'] div[class='card-header border-bottom rounded-0'] span"
      ),
    lblWarning: () =>
      cy
        ...
        .should("be.visible")
        .contains(
          "It is required to setup a payment method before creating integrations."
        ),
    lblWarning2: () =>
      cy
        .get("div[id='billing_payment-details'] p:nth-child(1)")
        .should("be.visible")
        .contains("You currently don't have a payment method."),
    btnPaymentMethod: () =>
      cy.get("form[aria-label='Set payment method'] button[class='button']"),
    lblSendingInvoiceTo: () =>
      cy.get("label[for='billing-sending_invoices_to'] span"),
    inputDisableSendingInvoiceTo: () => cy.get("#billing-sending_invoices_to"),
    lblCCInvoiceTo: () => cy.get("label[for='billing-cc_invoices_to'] span"),
    inputCCInvoiceTo: () => cy.get("#billing-cc_invoices_to"),
    lblVATNumber: () => cy.get("label[for='billing-vat_id'] span"),
    inputVATNumber: () => cy.get("#billing-vat_id"),
  ...
  verifyLabel = () => {
    ...
    this.elements
      .lblFooter()
      .should("be.visible")
      .contains("* All prices are excluding applicable VAT.");
    this.elements
      .lblPaymentInfo()
      .should("be.visible")
  ...
  afterPayRedirectBackToWebsite = () => {
    cy.visit(
      "/dashboard/account/billing?payment-method-status=expired#/payment-method"
    );
  };
  //pay by using card
  paymentMolliebyCard = () => {
    ...
    this.elements
      ...
      .should("not.be.empty")
      .then((body) => {
        cy.wrap(body).find("#cardHolder").type("Test visa", { force: true });
      });
    this.elements
      .inputExpiryDate()
      .its("0.contentDocument.body")
      .should("not.be.empty")
      .then((body) => {
        cy.wrap(body).find("#expiryDate").type("1030", { force: true });
      });
    this.elements
      .inputcvv()
      .its("0.contentDocument.body")
      .should("not.be.empty")
      .then((body) => {
        cy.wrap(body).find("#verificationCode").type("234", { force: true });
      });
  ...
  checkRequiredFields = () => {
    ...
    this.elements
      .errormsg()
      .should("be.visible")
      .contains("The billing cc invoices to must be a valid email address.");
    this.elements
      .errormsg()
      .should("be.visible")
      .contains("This VAT identification number is invalid.");
  };
  ...
  clickDownloadButton = () => {
    cy.window()
      ...
      .then(function (doc) {
        ...
        cy.get("td[class='text-right'] a[class='button']")
          .eq(0)
          .should("be.visible")
